...
# coll.create_index([('employeeid', pymongo.ASCENDING),('date', pymongo.ASCENDING), ('intime', pymongo.ASCENDING)], unique=True)

#################################################
outfeed_path = '/DATA02/ml_Workspaces/FACE_RECOGNITION/out_feed_'
#path_of_test_images = outfeed_path + todays_data + "/"
path_of_test_images = outfeed_path + "/"
...
frames_write_path = '/DATA02/ml_Workspaces/FACE_RECOGNITION/Processed_Images_exit_'+'/'
results_path = '/DATA02/ml_Workspaces/FACE_RECOGNITION/FR_exit_results_' + '/'
...
class AsyncWriteDB(threading.Thread):
    ...
    def __init__(self, plbl, face_img, outtime):
        threading.Thread.__init__(self)
        self.plbl = plbl
    ...
    def run(self):
        ...
        mydoc = coll.find({'employeeid': e_id, 'date': todays_data})
        
        try:
            intime = mydoc[0]['intime']
            t1 = datetime.datetime.strptime(intime, "%H:%M")
            t2 = datetime.datetime.strptime(self.outtime, "%H:%M")
    
            twh = t2 - t1
            twh = str(twh)[:5]
            if twh[-1] == ':':
                twh = twh[:-1]
            print('total working hours = ', twh)
            myquery = {'employeeid': e_id, 'date': todays_data}
            newvalues = {"$set": {"outtime": outtime, 'totalworkhours': str(twh)}}
    
            coll.update_many(myquery, newvalues)
    
            cv2.imwrite(results_path + self.plbl + '_' + todays_data + '_' + self.outtime.replace(':', '-') + '_' + '.jpg',
                        face_img)
        except:
            pass
...
while True:
    ...
    if frame_cv2 is not None:
        ...
        frame_cv = frame_cv2[0:480,150:480]
        frameRGB = cv2.cvtColor(frame_cv, cv2.COLOR_BGR2RGB).tolist()
        frames_list.append(frameRGB)
        ...
        if len(frames_list) == 1:
            continue
            #process2 = Async_Process(frames_list[0], port="4106")
        else:
            process2 = Async_Process(frames_list[1], port="4106")
        process2.start()
